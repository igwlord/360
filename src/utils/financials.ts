import { Campaign, Transaction } from '../types/database.types';
...
/**
 * Calculates consolidated financial metrics for a project (Campaign/Event).
 * Includes child project aggregation and ROAS/ROI calculations.
 */
export const calculateFinancials = (project: Campaign, allProjects: Campaign[] = []): FinancialMetrics => {
    const transactions = project.transactions || [];

    // 1. Base Metrics (Own Data)
    const initialBudget = transactions
        .filter(t => t.type === 'initial' || (t.type as string) === 'adjustment')
        .reduce((acc, t) => acc + (t.amount || 0), 0);
        
    const ownRevenue = transactions
        .filter(t => t.type === 'income')
        .reduce((acc, t) => acc + (t.amount || 0), 0);
        
    const ownCost = transactions
    ...
    // Ensure ID comparison handles string/number mix safely
    const children = allProjects.filter(p => String(p.client || '') === String(project.id) || String((p as any).parent_id) === String(project.id));
    ...
    const roas = totalCost > 0 ? parseFloat((ownRevenue / totalCost).toFixed(2)) : 0;
    const roi = totalCost > 0 ? parseFloat(((ownRevenue - totalCost) / totalCost * 100).toFixed(1)) : 0;
